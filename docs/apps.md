# Apps

> - [Tools Collection](https://cloud.189.cn/web/share?code=Q3qMzqQ7vEnq)（访问码：ee66）
>
> - [Drivers Collection](https://cloud.189.cn/web/share?code=7rmyqaN7jENn)（访问码：1lrp）
>
> - [x-cmd](https://cn.x-cmd.com/)
>
> ```shell
> eval "$(curl https://get.x-cmd.com)"
> x upgrade
> ```
>

## 0-9

- [7-zip](https://www.7-zip.org/download.html)

## A

- [Adobe](mailto:euler0525@foxmail.com)

- [AriaNgGUI](https://github.com/Xmader/aria-ng-gui/releases)

- [AutoCAD](mailto:euler0525@foxmail.com)

- [Awe](https://sunlogin.oray.com/download/)

## B

- [BurpSuite](https://github.com/Euler0525/Burpsuite-Professional)

- [百度网盘](https://pan.baidu.com/download#win)

## C

- [Cadence](mailto:euler0525@foxmail.com)

- [CC Switch](https://github.com/farion1231/cc-switch/releases)

- [Clash for Windows](https://cloud.189.cn/web/share?code=MfQRvmqIZN7v)（访问码：qpu3）

- [Claude Code](https://claude.com/product/claude-code)

- [CMake](https://cmake.org/download)

- [Codex](https://chatgpt.com/codex)

- [ctags](https://github.com/universal-ctags/ctags)

---

- [CTF Tools Collection](https://cloud.189.cn/web/share?code=Zn2MrqJZrYZb)（访问码：3j1b）

## D

- [DingTalk](https://www.dingtalk.com/en)

- [Docker Desktop](https://docs.docker.com/desktop/install/windows-install/)

- [Dynamic Theme](https://apps.microsoft.com/detail/9nblggh1zbkw?hl=zh-CN&gl=CN)

## E

- [EasyTshark](https://www.xuanyuancode.com/easytshark)

- [Edraw](https://www.edrawsoft.cn/download/)

- [Everything](https://www.voidtools.com/downloads/)

## F

- [File Converter](https://github.com/Tichau/FileConverter/releases)

- [FFmpeg](https://ffmpeg.org/download.html)

- [Fluent Reader](https://github.com/yang991178/fluent-reader/releases)

- [Fluent Reader Lite](https://github.com/yang991178/fluent-reader-lite/releases)

- [FreeCommander](https://freecommander.com/en/downloads/)

## G

- [GeoGebra](https://www.geogebra.org/download)

- [Git](https://git-scm.com/install/windows)

- [Google AI Studio](https://github.com/homielab/ai-studio-desktop/releases)

## H

- [火绒安全](https://www.huorong.cn/)

- [Hiddify](https://hiddify.com/)

## I

- [IDA Pro](https://pan.baidu.com/s/1IMTKHORAK8rNjjPZBVivmw?pwd=qcas)

- [IDM](https://www.internetdownloadmanager.com/download.html)

- [iFlow CLI](https://cli.iflow.cn/)

- [iTubeGo](https://cloud.189.cn/web/share?code=6ruiA3UJzuQr)（访问码：wz0q）

## J

## K

- [Kiro](https://kiro.dev/downloads/)

## L

- [Lazygit](https://github.com/jesseduffield/lazygit)

- [lx-music](https://github.com/lyswhut/lx-music-desktop/releases), [lx-musc-source](https://github.com/Euler0525/lx-music-source)

## M

- [Manjaro](https://sourceforge.net/projects/manjarolinux/files/)

- [MathType](https://cloud.189.cn/web/share?code=AfieIrNrm2U3)（访问码：xtq8）

- [MATLAB](mailto:euler0525@foxmail.com)

- [Maple](mailto:euler0525@foxmail.com)

- [Microsoft Office](https://zhuanlan.zhihu.com/p/661035592)

- [MinGW](https://github.com/niXman/mingw-builds-binaries/releases)

- [MkDocs](https://www.mkdocs.org/user-guide/installation/)

```shell
pip install mkdocs
pip install mkdocs-material
```

- [Motrix](https://motrix.app/)

- [MobaXterm](https://mobaxterm.mobatek.net/download.html)
    - [MobaXterm license Generate](https://inused.github.io/pages/file/tool/MobaXtermKeygen.html)
- [ModelSim](mailto:euler0525@foxmail.com)

## N

- [NetAssit](https://cmsoft.cn/resource/102.html)

- [Nodepad++](https://notepad-plus-plus.org/downloads/)

- [NVIDIA](https://www.nvidia.cn/geforce/drivers/)

- [nvm](https://github.com/coreybutler/nvm-windows/releases)

- [匿名助手](mailto:euler0525@foxmail.com)

## O

- [OBS Studio](https://obsproject.com/download)

- [Oracle_Database_11g](mailto:euler0525@foxmail.com)

## P

- [PalmInput](https://www.xinshuru.com/index.html?p=win)

- [PicGo](https://github.com/Molunerfinn/PicGo/releases/tag/v2.3.1)

- [PicPick](https://picpick.app/en/download/)

- [PotPlayer](https://potplayer.daum.net/)

- [PowerToys](https://learn.microsoft.com/en-us/windows/powertoys/install)

- [Pytorch for GPU](https://pytorch.org/)

```python
import torch


def print_gpu_info():
    gpu_count = torch.cuda.device_count()
    print(f"系统中的 GPU 数量: {gpu_count}")

    for i in range(gpu_count):
        device_name = torch.cuda.get_device_name(i)
        print(f"\nGPU {i+1} 名称: {device_name}")

        if i == torch.cuda.current_device():
            print("当前正在使用该 GPU")

        memory_allocated = torch.cuda.max_memory_allocated(i) / (1024 ** 3)
        memory_cached = torch.cuda.max_memory_reserved(i) / (1024 ** 3)
        print(
            f"GPU {i+1} 内存使用情况: 已分配 {memory_allocated:.2f}GB，缓存 {memory_cached:.2f}GB")


print_gpu_info()
```

## Q

- [qBittorrent](https://www.qbittorrent.org/download)

- [Quarto](https://quarto.org/docs/download/)

- [QuickLook](https://apps.microsoft.com/detail/9nv4bs3l1h4s?launch=true&mode=full&hl=en-us&gl=cn&ocid=bingwebsearch)

## R

- [RaiDrive](https://www.raidrive.com/download)

- [Rust](https://www.cnblogs.com/hachiroku/p/19148749)

## S

- [SanDisk](mailto:euler0525@foxmail.com)

- [Serial Debug Assistant](https://apps.microsoft.com/detail/9nblggh43hdm?launch=true&mode=full&hl=en-us&gl=cn&ocid=bingwebsearch)

- [Serial Studio](https://serial-studio.com/download)

- [Shadowsocks](https://github.com/shadowsocks)

- [SMC](http://shanmiaocloud.com/)

- [Sniffnet](https://sniffnet.net/)

- [Spring Boot](https://euler0525.github.io/blogs/posts/23335ee9/)

- [SuperCom](https://github.com/SuperStudio/SuperCom)

- [STM32CubeIDE](https://www.st.com/en/development-tools/stm32cubeide.html)

- [STM32CubeCLT](https://www.st.com/en/development-tools/stm32cubeclt.html)

- [STM32CubeMX](https://www.st.com/en/development-tools/stm32cubemx.html)

- [STMCUFinder](https://www.st.com/en/development-tools/st-mcu-finder-pc.html)

## T

- [Tex Live](https://tug.org/texlive/acquire-netinstall.html)

- [Thunderbird](https://www.thunderbird.net/en-US/download/)

- [tldr](https://tldr.sh/)

- [Typora](https://typoraio.cn/), [typora_plugin](https://github.com/obgnail/typora_plugin)

```shell
# ./resources/plugin/global/settings/custom_plugin.default.toml
MD007 = { indent = 4 }
```

- `tcping`

```shell
tcping -s               快速显示端口连通性
tcping -t               持续发送数据包，Ctrl+C停止
tcping -n 5     指定发送5个包结束
tcping -d               在每行前面附加日期时间
tcping -r 3     每发送3个数据包，重新查找主机一次
```

## U

- [UartAssist](https://cmsoft.cn/resource/101.html)

## V

- [vcxsrv](https://github.com/marchaesen/vcxsrv/releases)

- [Visual Studio](https://visualstudio.microsoft.com/downloads/)

```powershell
winget show --versions "Microsoft.VisualStudio.2022.Community"
winget install -v <17.14.24> "Microsoft.VisualStudio.2022.Community"
```

- [VS Code](https://code.visualstudio.com/Download)
- [VMware](https://pc.qq.com/detail/0/detail_21600.html)

> ZF3RO-FHED2-M80TY-8QYGC-NPKYF
>
> YF390-OHF8P-M81RQ-2DXQE-M2UT6
>
> ZF71R-DMX85-08DQY-8YMNC-PPHV8

- [VOFA+](https://www.vofa.plus/)

## W

- [WinSCP](https://winscp.net/eng/download.php)

- [Wireshark](https://www.wireshark.org/download.html)

- [WPS](https://platform.wps.cn/)

- [WSL](https://learn.microsoft.com/en-us/windows/wsl/install)

- [WeChat](https://windows.weixin.qq.com/?lang=zh_CN)

- [WeMeet](https://meeting.tencent.com/download/)

## X

- [XcXsrv](https://sourceforge.net/projects/vcxsrv/)

- [Xilinx-Vivado Design Tools](https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/vivado-design-tools.html)

```bat
@rem vitis.bat 去除第41行末尾空格
41     set XILINX_VIVADO=%RDI_INSTALLROOT%/Vivado/%RDI_INSTALLVER%
```

## Y

- [Yakit](https://yaklang.com/)

## Z

- [ZTools](https://github.com/ZToolsCenter/ZTools/releases)

- [ZoomIt](mailto:euler0525@foxmail.com)

- [Zotero](https://www.zotero.org/download/)

---

## 现存可用网盘概览

> 仅包含国内网盘，排名不分先后

|     名称     | 上传 | 下载 | 容量 |          特点          |       用途       |
| :----------: | :--: | :--: | :--: | :--------------------: | :--------------: |
|   阿里云盘   | < 10MB/s | < 1MB/s | 680GB | 支持 NAS 定期自动同步 | NAS 文件备份 |
|    蓝奏云    | < 5MB/s | - | 10GB | 分享大文件需会员权限 | 资源分享外链 |
|   天翼云盘   | 上传需等待网络 | < 20MB/s | 30GB | 分享文件需审核 | 资源分享外链 |
|   百度网盘   | - | × | 2TB | 垄断…… |     获取资源     |
|   城通网盘   | - | < 1MB/s | - | 网络资源 | 获取资源 |
| 移动云盘 | - | - | 20GB | 文件格式有限制，手机端专用 | 后台备份手机文件 |
|   WPS 云盘    |  -   |  -   | 1TB  | 支持在线预览、多端同步 |   存储所有文档   |
|     钉盘     |  -   |  -   | 2GB  |        学校专用        | 学校课程资料暂存 |
|   夸克网盘   | - | < 500KB/s | 10GB | 需要手机夸克 | × |
|   腾讯微云   | < 5MB/s | < 3MB/s | 10GB | 速度较慢 | × |
|   迅雷云盘   | - | × | 10GB | 不会下载（连接资源中……） | × |
|123 云盘|< 5MB/s|-|2TB|广告太多，不支持 Alist|×|
|   115 网盘    | < 200MB, < 10MB/s | < 200KB/s | 15GB | 强制使用 115 浏览器 | × |

