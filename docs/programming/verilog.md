...
```verilog
localparam integer SIGNAL_LEN = 4096;
localparam integer DATA_WIDTH = 16;
localparam integer IDX_WIDTH  = 16;

reg  [IDX_WIDTH-1:0]  idx;
reg  [DATA_WIDTH-1:0] signal_txt [0:SIGNAL_LEN-1];
reg  [DATA_WIDTH-1:0] signal;

initial begin
    idx    <= '0;
    signal <= '0;
    $readmemh("C:/Syncdisk/f.txt", signal_txt);
end
...
```matlab
DATA_WIDTH = 16
signal_12bits = round((signal / max(abs(signal))) * 32767);
signal_12bits_hex = cellstr(dec2hex(mod(signal_12bits, 2 ^ 16), 4));

fid = fopen('C:/Syncdisk/signal_12bits_hex.txt', 'w');
fprintf(fid, '%s\n', signal_12bits_hex{:});
fclose(fid);
```
