...
```python
"""
PyTorch Floating Point Precision and Associativity Demo.

This script demonstrates:
1. Floating-point non-associativity in matrix operations.
2. Precision differences between CPU and GPU (specifically regarding TF32).
"""

import torch

# Dimension for the square matrices
DIM = 1024
...
A1, B1 = torch.randn(DIM, DIM), torch.randn(DIM, DIM)
A2, B2 = torch.randn(DIM, DIM), torch.randn(DIM, DIM)
A3, B3 = torch.randn(DIM, DIM), torch.randn(DIM, DIM)

P1 = A1 @ B1
P2 = A2 @ B2
P3 = A3 @ B3

# Test floating-point associativity: (A + B) + C vs A + (B + C)
res_order1 = (P1 + P2) + P3
res_order2 = P1 + (P2 + P3)

# Calculate differences
is_exact_equal = (res_order1 == res_order2).all().item()
max_diff_assoc = (res_order1 - res_order2).abs().max().item()
diff_count_assoc = (res_order1 != res_order2).sum().item()

print(f"Bitwise exact match: {is_exact_equal}")
print(f"Number of mismatched elements: {diff_count_assoc} / {DIM * DIM}")
print(f"Maximum absolute error: {max_diff_assoc:.10f}")

# Check GPU availability and compare CPU vs GPU precision
if torch.cuda.is_available():
    # Disable TF32 for higher precision baseline
    torch.backends.cuda.matmul.allow_tf32 = False
    torch.backends.cudnn.allow_tf32 = False

    A_gpu, B_gpu = A1.cuda(), B1.cuda()
    C_gpu_fp32 = (A_gpu @ B_gpu).cpu()
    C_cpu = A1 @ B1

    diff_fp32 = (C_cpu - C_gpu_fp32).abs()
    print(
        f"[TF32 Disabled] CPU vs GPU Max Error: {diff_fp32.max().item():.10f}")
    print(
        f"[TF32 Disabled] Mismatched elements: {(C_cpu != C_gpu_fp32).sum().item()} / {DIM * DIM}")

    # Enable TF32 (default on Ampere+ architectures for speed)
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cudnn.allow_tf32 = True

    C_gpu_tf32 = (A_gpu @ B_gpu).cpu()

    diff_tf32 = (C_cpu - C_gpu_tf32).abs()
    print(
        f"[TF32 Enabled] CPU vs GPU Max Error: {diff_tf32.max().item():.10f}")
    print(
        f"[TF32 Enabled] Mismatched elements: {(C_cpu != C_gpu_tf32).sum().item()} / {DIM * DIM}")
...
py_compile.compile("test.py", "output.pyc")

compileall.compile_dir('./your_project', force=True)  # Ignore the existing .pyc
```

或使用命令行方式

```shell
python -m compileall ./your_project
```
...
text = '\n'.join(f"{instr.offset:3d} {instr.opname:20s} {instr.argval}" 
                 for instr in instructions)

with open('bytecode.txt', 'w', encoding='utf-8') as f:
    f.write(text)
```
